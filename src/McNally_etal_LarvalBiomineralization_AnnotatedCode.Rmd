...
LarvByJar$v1_5SurvCount<- rowSums(LarvByJar[,c("TotalRTotal", "F2LarvaeCount")], na.rm=TRUE)
...
p1=visreg(larvgrowfin, "JarSatArg_Total", by="ParentTrt", overlay=TRUE, ylab=expression(paste("Growth (", mu, "m/day)")), xlab=expression(paste("Larvae Treatment ", Omega[Aragonite])), legend=FALSE)
par(mfcol=c(2,2))
p2<-visreg(larvlenfin, "JarSatArg_Total", by="ParentTrt", overlay=TRUE, ylab=expression(paste("Length (", mu, "m)")), xlab="", legend=FALSE)
p3<-visreg(larvareafint, "JarSatArg_Total", by="ParentTrt", overlay=TRUE, ylab=expression(sqrt(paste("Area (", mu, m^2,")"))), xlab=expression(paste("Larvae Treatment ", Omega[Aragonite])), legend=FALSE)
p4<-visreg(larvperimfin, "JarSatArg_Total", by="ParentTrt", overlay=TRUE, ylab=expression(paste("Perimeter (", mu, "m)")), xlab="", legend=FALSE)
p5<-visreg(PerDiRatFin, "JarSatArg_Total", by="ParentTrt", overlay=TRUE, ylab="Perimeter :Length", xlab=expression(paste("Larvae Treatment ", Omega[Aragonite])), legend=FALSE)
```
...
par(mfcol=c(2,2))
ggplot(LarvByJarVarVizDat, aes(y=MeanGPD,x=as.factor(MaleIDPlain), color=JarTrt))+
...
Anova(lm(RatSurv ~ MeanGPDExp, data=ExpFams, type=2))
...
EPmodlm<- lm(RatSurv~MeanGPDExp, data=ExpFams)
EPmodlmsimp<- lm(RatSurv~1, data=ExpFams)
AIC(EPmodlm)
AIC(EPmodlmsimp)
par(mfcol=c(2,2))
plot(EPmodlm)
EPmodlmcon<- lm(RatSurv~MeanGPDExp, data=ConFams)
EPmodlmconsimp<- lm(RatSurv~1, data=ConFams)
AIC(EPmodlmcon)
AIC(EPmodlmconsimp)
plot(EPmodlmcon)
par(mfcol=c(1,1))
Anova(EPmodlm, type="II")
Anova((EPmodlmcon), type="II")
...
plot(RatSurv~MeanGPDExp, data=ConFams, pch=19, col="deepskyblue", xlim=c(-2, 2.4), ylim=c(0.6, 1.4), xlab="Mean Family Growth (um/day)", ylab="Proportion of Surviving Larvae \n (OA:Control)")
...
points(RatSurv~MeanGPDExp, data=ConFams, pch=19, col="deepskyblue")
points(RatSurv~MeanGPDExp, data=ExpFams, pch=19, col="brown1")
...
```{r}
OALarvae<- subset(LarvByJarDat, JarTrt=="Exposed", select=c(JarTrt, CrossID, ProtocolVersion, LarvaeSurvived))
controlLarvae<- subset(LarvByJarDat, JarTrt== "Control", select=c(JarTrt, CrossID, ProtocolVersion, LarvaeSurvived))
boxplot(LarvaeSurvived~CrossID*ProtocolVersion, data=OALarvae, col="gray45", ylim=c(0,2500))
par(new=T)
boxplot(LarvaeSurvived~CrossID*ProtocolVersion, data=controlLarvae, col="white", ylim=c(0,2500))

#run a paired t test to see if protocol version impacts 
ProtDattoWide<- subset(LarvByJarDat, select=c(ProtocolVersion, CrossID, JarTrt, LarvaeSurvived), sep="_")

OALarvaeProt1<- subset(OALarvae, ProtocolVersion!=2)
colnames(OALarvaeProt1)<- paste(c("JarTrt", "CrossID", "ProtVers", "LSurvivedProt1"))
OALarvaeProt2<- subset(OALarvae, ProtocolVersion==2)
colnames(OALarvaeProt2)<- paste(c("JarTrt", "CrossID", "ProtVers2", "LSurvivedProt2"))
OALarv<- merge(OALarvaeProt1, OALarvaeProt2, by=c("CrossID", "JarTrt"))
OALarvMeans<- ddply(OALarv, .(CrossID), FUN=mean)
t.test(OALarvMeans$LSurvivedProt1, OALarvMeans$LSurvivedProt2, paired=T)
qqnorm((OALarvMeans$LSurvivedProt1-OALarvMeans$LSurvivedProt2))
qqline((OALarvMeans$LSurvivedProt1-OALarvMeans$LSurvivedProt2))
OALarvMeans$Diffs<- (OALarvMeans$LSurvivedProt1-OALarvMeans$LSurvivedProt2)
OALarvMeans<- subset(OALarvMeans, Diffs != "NA")
OALarvMeans$PercDiff<- (OALarvMeans$Diffs/OALarvMeans$LSurvivedProt2)*100

ConLarvProt1<- subset(controlLarvae, ProtocolVersion !=2)
ConLarvProt2<- subset(controlLarvae, ProtocolVersion ==2)
colnames(ConLarvProt1)<- paste(c("JarTrt", "CrossID", "ProtVers", "LSurvivedProt1"))
colnames(ConLarvProt2)<- paste(c("JarTrt", "CrossID", "Protvers2", "LSurvivedProt2"))
ConLarv<- merge(ConLarvProt1, ConLarvProt2, by= c("JarTrt", "CrossID"))
t.test(ConLarv$LSurvivedProt1, ConLarv$LSurvivedProt2, paired=T)
qqnorm((ConLarv$LSurvivedProt1-ConLarv$LSurvivedProt2))
qqline((ConLarv$LSurvivedProt1-ConLarv$LSurvivedProt2))
