...
survfin_Prot2<- lm(RatSurv~ 1, data=SurvWideDatProt2)#final model chosen by step function: RatSurv~ 1
plot(survfin_Prot2)
...
#use a paired t-test to test for protocol 1 vs 2 differences for families
#get the data into a single dataframe need: cross ID, jar treatment, larvae count
SurvTtestexp<- data.frame(CrossID= SurvWideDatProt1$CrossID, JarTrt="Exposed", Prot1Count= SurvWideDatProt1$MeanLarvSurvivedExp)
SurvDatProt2exp<- filter(SurvDatProt2, JarTrt=="Exposed")
SurvTtestexp$Prot2Count<- SurvDatProt2exp$LarvaeSurvived[match(SurvDatProt2exp$CrossID, SurvTtestexp$CrossID)]

SurvTtestDat<- data.frame(CrossID= SurvWideDatProt1$CrossID, JarTrt="Control", Prot1Count= SurvWideDatProt1$MeanLarvSurvivedCon)
SurvDatProt2con<- filter(SurvDatProt2, JarTrt=="Control")
SurvTtestDat$Prot2Count<- SurvDatProt2con$LarvaeSurvived[match(SurvDatProt2con$CrossID, SurvTtestDat$CrossID)]
#put the two protocol 1 data frames together
SurvTTest<- rbind(SurvTtestexp, SurvTtestDat)
SurvTTest$P1P2Diff<- SurvTTest$Prot1Count- SurvTTest$Prot2Count
qqnorm(SurvTTest$P1P2Diff) #check for normality
qqline(SurvTTest$P1P2Diff)

t.test(SurvTTest$Prot1Count,SurvTTest$Prot2Count, paired=T)
```
