...
HeaderB1<- subset(Header_WC, BlockID=="B1" & HeadVindtaTemp !="NA", select=c(Treatment, HeadSatArg_Total, HeadpHTotal))
HeaderB1$JarType<- paste("Header")
colnames(HeaderB1)<- paste(c("Trt", "JarSatArg_Total", "JarpHTotal", "JarType"))

ControlJars<- Larvae_WC
ControlJars$BlockID<- vapply(strsplit(as.character(ControlJars$JarID),"_"), `[`, 2, FUN.VALUE=character(1))
ControlJars<- subset(ControlJars, BlockID=="B1" & ControlJar=="TRUE" & JarpHTotal !="NA", select=c(Trt, JarSatArg_Total, JarpHTotal))
...
boxplot(JarpHTotal~JarType, data=JarWaterCon, ylab="pH total", xlab="Jar Type", ylim=c(6,8.1))
par(new=TRUE)
boxplot(JarpHTotal~JarType, data=JarWaterExp, ylab="", xlab="", ylim=c(6,8.1))
```
...
JarMeans<- ddply(CarbChemJar, .(Trt), numcolwise(mean, na.rm=T))
#write.csv(JarCarbChem,"~/R/GitHub/LarvaeTransGen2018/data/JarCarbChemSummary_5-26.csv")
```
...
par(mfcol=c(4,1), mar=c(2,4.6, .1,1), xpd=TRUE)
...
p2<-visreg(larvlenfin, "JarSatArg_Total", by="ParentTrt", overlay=TRUE, ylab=expression(paste("Length (", mu, "m)")), xlab="", legend=FALSE, line=list(col=c( "#F7645A", "#004D4F"), lwd=c(2,2)),
              fill=list(col=c("#FCCECB", "#3BFAFF")),
        points=list(col=c("#F98880", "#007367"), pch=19, cex=0.4))
...
ggplot(data = FamMeansWide) +                        # Set data
  ...
  ggplot(data = FamMeansWide) +                        # Set data
  geom_tile(aes(x = FemaleIDNumber, y = MaleIDNumber, fill = OACont_Len)) +        # Define variables to plot
  scale_fill_gradient2(low = "darkred", mid="violet", high = "navyblue", midpoint=mean(ConFamMeansWide$OACont_Len))+  # Set colour scheme
  facet_grid(~ParTrt, scale="free")+
  labs(x="Female ID", y= "Male ID", fill= "Length Difference \n (OA-Control)")+
  theme_classic()
...
```
## Elise Playing around:
```{r}
p2<-visreg(larvlenfin, "JarSatArg_Total", by="ParentTrt", overlay=TRUE, ylab=expression(paste("Length (", mu, "m)")), xlab="", legend=FALSE, line=list(col=c( "#F7645A", "#004D4F"), lwd=c(2,2)),
              fill=list(col=c("#FCCECB", "#3BFAFF")),
        points=list(col=alpha(c("white", "white"), 0), pch=19, cex=0.4))
...
p3<-visreg(larvareafint, "JarSatArg_Total", by="ParentTrt", overlay=TRUE, ylab=expression(sqrt(paste("Area (", mu, m^2,")"))), xlab=expression(paste("Larvae Treatment ", Omega[Aragonite])), legend=FALSE, cex=0.6, line=list(col=c( "#F7645A", "#004D4F"), lwd=c(2,2)),
              fill=list(col=c("#FCCECB", "#3BFAFF")),
        points=list(col=alpha(c("white", "white"), 0), pch=19, cex=0.4))
p4<-visreg(larvperimfin, "JarSatArg_Total", by="ParentTrt", overlay=TRUE, ylab=expression(paste("Perimeter (", mu, "m)")), xlab="", legend=FALSE, cex=0.6, line=list(col=c( "#F7645A", "#004D4F"), lwd=c(2,2)),
              fill=list(col=c("#FCCECB", "#3BFAFF")),
        points=list(col=alpha(c("white", "white"), 0), pch=19, cex=0.4))
p5<-visreg(PerDiRatFin, "JarSatArg_Total", by="ParentTrt", overlay=TRUE, ylab="Perimeter :Length", xlab=expression(paste("Larvae Treatment ", Omega[Aragonite])), legend=FALSE, cex=0.6, line=list(col=c( "#F7645A", "#004D4F"), lwd=c(2,2)),
              fill=list(col=c("#FCCECB", "#3BFAFF")),
        points=list(col=alpha(c("white", "white"), 0), pch=19, cex=0.4))
```
