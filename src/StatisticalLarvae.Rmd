...
# Tests on length
```{r}
length1 = lmer(log(MaxFeretDiamum) ~ JarTrt * ParentTrt + (1|CrossID:JarID) + (1|FemaleID), data = larvae)

# Check model assumptions
qqnorm(resid(length1))
qqline(resid(length1))

length2 = lmer(log(MaxFeretDiamum) ~ JarTrt + ParentTrt + (1|CrossID:JarID) + (1|FemaleID), data = larvae)
anova(length1, length2) # Significant interaction, go with more complicated model

length3 = lmer(log(MaxFeretDiamum) ~ 1 * ParentTrt + (1|CrossID:JarID) + (1|FemaleID), data = larvae)
