...
larvae <- read.csv("~/Documents/GitHub/LarvaeTransGen2018/data/Larvae_morphology.csv")
```
Tests on mass per larvae
```{r}
# Male should be Random effect but wont converge so leaving off for now
area1 = lmer(sqrt(SurfaceAreaum2) ~ JarTrt * ParentTrt + (1|JarID) + (1|CrossID) +  (1|FemaleID) , data = larvae)
qqnorm(resid(area1))
qqline(resid(area1))

area2 = lmer(sqrt(SurfaceAreaum2) ~ JarTrt + ParentTrt + (1|JarID) + (1|CrossID) +  (1|FemaleID) , data = larvae)
anova(area1,area2) # Significant interaction so we go with more complicated model

area3 = lmer(SurfaceAreaum2 ~ 1 * ParentTrt + (1|JarID) + (1|CrossID) +  (1|FemaleID) , data = larvae)
anova(area1,area3) #Significant jar effect (larval treatment)

area4 = lmer(SurfaceAreaum2 ~ JarTrt * 1 + (1|JarID) + (1|CrossID) +  (1|FemaleID) , data = larvae)
