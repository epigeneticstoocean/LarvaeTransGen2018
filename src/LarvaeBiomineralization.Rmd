...
#look at assumptions of eggarea5
plot(eggarea5)
qqnorm(resid(eggarea5))
qqline(resid(eggarea5))
...
#look at the assumptions of eggeccen5
plot(eggeccen5)
qqnorm(resid(eggeccen5))
qqline(resid(eggeccen5))
...
boxplot(JarOmegaCalcite~Block*JarTrt, data=LarvByJarDat)
...
#plot just by JarTrt
ggplot(data = meaneccen, aes(x = JarTrt, y = LarvaeEccentricity)) +
  geom_bar(stat="identity", aes(fill="gray45"), position="dodge")+
  labs(x = "Parent Treatment", y = "Larvae Eccentricity") +
...
Look at egg data with outliers included. 
```{r}
#start with models that have the response variable EggDiamum
eggdiamY1<- lmer(EggDiamum~WCa_pHDIC*AdultLength + (1|FemaleID) + (1|Block) +(1|AccTankID), data=EggsYoutliers)

#Check assumptions
par(mfcol=c(1,1))
qqnorm(resid(eggdiamY1))
qqline(resid(eggdiamY1))
plot(eggdiamY1)
acf(eggdiamY1)

summary(eggdiamY1)
#try simplifying the model; step function gives me errors, so I ended up doing this manually
eggdiamY2<- lmer(EggDiamum~WCa_pHDIC+ (1|FemaleID) + (1|Block) , data=eggdiamY1)
anova(eggdiam1, eggdiam2)#Choose eggdiam2
eggdiam3<- lm(EggDiamum~WCa_pHDIC, data=CalEggs)
anova(eggdiam2, eggdiam3)#choose eggdiam2
eggdiam4<- lmer(EggDiamum~WCa_pHDIC + (1|FemaleID), data=CalEggs)
...
#try simplifying the model; step function gives me errors, so I ended up doing this manually
eggarea2<- lmer(EggAreaum2~WCa_pHDIC+ (1|FemaleID) + (1|Block) , data=EggsYoutliers)
anova(eggarea1, eggarea2)#Choose eggarea2
eggarea3<- lm(EggAreaum2~WCa_pHDIC, data=CalEggs)
anova(eggarea2, eggarea3)#choose eggarea2
eggarea4<- lmer(EggAreaum2~WCa_pHDIC + (1|FemaleID), data=CalEggs)
