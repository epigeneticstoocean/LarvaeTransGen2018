...
LarvByJarDat$EggCorrection<- mean(meanegg$EggDiamum)/LarvByJarDat$EggDiamum#correction factor for the rank of the egg size for that dam
LarvByJarDat$GPDCorr<- (LarvByJarDat$LarvaeDiamum*LarvByJarDat$EggCorrection)/3
```
...
#no significant difference between header and larvae jars = no significant drift. 
...
# Look at relationship between egg size and shell length
```{r}
#use just control larvae exposure
LarvaeDatControl<- subset(LarvaeDat, JarTrt=="Control")
LarvLEggD<- lm(LarvaeDiamum~EggDiamum, data=LarvaeDatControl)
par(mfcol=c(2,2))
plot(LarvLEggD)
par(mfcol=c(1,1))
acf(LarvLEggD$residuals)
summary(LarvLEggD)#there is a significant relationship between egg size and growth

plot(LarvaeDiamum~EggDiamum, data=LarvaeDatExposed)

#use just control larvae exposure
LarvaeDatExposed<- subset(LarvaeDat, JarTrt=="Exposed")
LarvLEggDexp<- lm(LarvaeDiamum~EggDiamum, data=LarvaeDatExposed)
par(mfcol=c(2,2))
plot(LarvLEggD)
par(mfcol=c(1,1))
acf(LarvLEggD$residuals)
summary(LarvLEggDexp)

#use the mean egg diameter for both parental treatments to normalize larvae length by egg size then calculate growth per day
LarvaeDat$EggCorrection<- mean(meanegg$EggDiamum)/LarvaeDat$EggDiamum#correction factor for the rank of the egg size for that dam
LarvaeDat$GPDCorr<- (LarvaeDat$LarvaeDiamum*LarvaeDat$EggCorrection)/3
```
...
# GPD Stats 
```{r LarvGrowthStat}
#first calculate with GPD calculated as: (L-E)/3
...
#Now calculate with GPD calculated as: (L*EggCorrection)/3
larvgrowcorr<-lmer(GPDCorr~ParentTrt*JarSatArg_Total+(1|MaleID)+(1|CrossID)+(1|JarID) +(1|JarSeatable)+(1|TrtTankID)+(1|MaleTrtTankID), data=LarvaeDat)
steppedlarvgrowcorr<- step(larvgrowcorr)
print(steppedlarvgrowcorr)
#final model chosen by the lme4 step function: y~ParentTrt*JarSatArg+ (1|CrossID)+ (1|JarID) + (1|JarSeatable)
larvgrowcorrfin<- lmer(GPDCorr~ParentTrt*JarSatArg_Total+(1|CrossID)+ (1|JarID) + (1|JarSeatable), data=LarvaeDat)
#check the final model to see that it meets assumptions
plot(larvgrowcorrfin) #seems to meet assumption of linearity and homoscedasticity
qqnorm(resid(larvgrowcorrfin))
qqline(resid(larvgrowcorrfin)) #has relatively long tails, based on scale I think it reasonably meets assumption of normality. 
acf(resid(larvgrowcorrfin))

summary(larvgrowcorrfin)

ggplot(LarvaeDat, aes(x=GPDCorr,color=JarTrt))+
  geom_histogram(alpha=0.5, position="identity")+
  facet_grid(ParentTrt~.)
...
summary(PerDiRatFin)
r.squaredGLMM(PerDiRatFin) #use R2c
```
...
r.squaredGLMM(larvgrowcorrfin) #GPD R2 use R2c
...
par(mfcol=c(2,2), mar=c(2.5,2, .1,.5), xpd=TRUE, pty="s")
p1<- visreg(larvgrowcorrfin, "JarSatArg_Total", by="ParentTrt", overlay=TRUE, ylab=expression(paste("Growth (", mu, "m/day)")), xlab="", legend=FALSE, line=list(col=c( "#F7645A", "#004D4F"), lwd=c(2,2)),
...
p2<-visreg(larvlenfin, "JarSatArg_Total", by="ParentTrt", overlay=TRUE, ylab=expression(paste("Length (", mu, "m)")),  xlab=expression(paste("Larvae Treatment ", Omega[Aragonite])), legend=FALSE, line=list(col=c( "#F7645A", "#004D4F"), lwd=c(2,2)),
              fill=list(col=c("#FCCECB", "#3BFAFF")),
        points=list(col=c("#F98880", "#007367"), pch=19, cex=0.4))
...
FamMeansWide<- pivot_wider(FamMeans, id_cols= CrossID, names_from= JarTrt, values_from= c(MeanGPD, GPDCorr, LarvaeAreaum2, LarvaeDiamum, LarvaePerimeterum))

FamMeansWide$OACont_GPD<- (FamMeansWide$MeanGPD_Exposed-FamMeansWide$MeanGPD_Control)/FamMeansWide$MeanGPD_Control
FamMeansWide$OACont_GPDcorr<- (FamMeansWide$GPDCorr_Exposed-FamMeansWide$GPDCorr_Control)/FamMeansWide$GPDCorr_Control
