...
Compile all of the data for the water chemistry summary table. 
...
## Test for significant effect of parent treatment on egg size before removing outliers. 
```{r EggSizeStat AllEggs}
CalEggs<- EggsB1
...
boxplot(EggDiamum~FemaleID, data=EggsB1) 
EggsB1$ParentTrt<- factor(EggsB1$ParentTrt, levels=c("400", "2600"))
boxplot(EggDiamum~ParentTrt, data=EggsB1, col= c("#004D4F", "#F7645A"))
...
## Make datasets with mean egg size after egg outliers were removed
...
## Compare Header, Control Jar, and Experiment Jar pH
```{r JarChem Comparison to header tanks}
#subset main water chem dataframes to just get the bottles we are interested in.
HeaderB1<- subset(Header_WC, Date==20180709 , select=c(Treatment, HeadSatArg_Total, HeadpHTotal))
HeaderB1$JarType<- paste("Header")
colnames(HeaderB1)<- paste(c("Trt", "JarSatArg_Total", "JarpHTotal", "JarType"))


LarvaeJars<- Larvae_WC
LarvaeJars$BlockID<- vapply(strsplit(as.character(LarvaeJars$JarID),"_"), `[`, 2, FUN.VALUE=character(1))
LarvaeJars<- subset(LarvaeJars, BlockID=="B1" & ControlJar=="FALSE" & JarpHTotal !="NA", select=c(Trt, JarSatArg_Total, JarpHTotal))
LarvaeJars$JarType<- paste("Larvae")
#combine the rows that have the treatment and the aragonite saturation state data
JarWaterChem<- rbind(HeaderB1,LarvaeJars)
...
jarmod<- t.test(JarSatArg_Total~ JarType, data=JarWaterExp, var.equal=T)
jarmod
shapiro.test(JarWaterExp$JarSatArg_Total)
leveneTest(data = JarWaterExp, JarSatArg_Total ~ JarType, center = mean)
#no significant difference between header and larvae jars for exposed larvae treatment = no significant drift. 

jarmod2<- t.test(JarSatArg_Total~ JarType, data=JarWaterCon, var.equal=T)
jarmod2
shapiro.test(JarWaterCon$JarSatArg_Total)
leveneTest(data = JarWaterCon, JarSatArg_Total ~ JarType, center = mean)
#no significant difference between header and larvae jars for control larvae treatment = no significant drift
boxplot(JarSatArg_Total~JarType, data=JarWaterCon, ylab="pH total", xlab="Jar Type", ylim=c(0,1))
par(new=TRUE)
boxplot(JarSatArg_Total~JarType, data=JarWaterExp, ylab="", xlab="", ylim=c(0,1))
```

## Get header carb chem summaries
...
HeaderB1all$Temp<- 20 #add a column for temp so that the function runs correctly headers were kept in temp controlled room. 
...
```{r Jar Carb Sum}
CarbChemJarAllBotts<- subset(LarvByJarDat, select= c("Trt", "JarpHTotal", "JarSalinity", "JarTemp", "JarDICCorr", "JarATCorr", "JarATCalc_Total", "JarpCO2_Total", "JarHCO3_Total", "JarCO3_Total", "JarCO2_Total", "JarSatCalcite_Total", "JarSatArg_Total" ))
colnames(CarbChemJarAllBotts)<- paste(c("Trt", "pH", "Sal", "Temp", "DIC", "ATmeas","ATcalc", "pCO2", "HCO3", "CO3", "CO2", "Calcite", "Aragonite"))
JarCarbChemAllBotts<- CarbChemSummary(DF=CarbChemJarAllBotts)

JarMeansAllBotts<- ddply(CarbChemJarAllBotts, .(Trt), numcolwise(mean, na.rm=T))
#write.csv(JarCarbChemAllBotts,"~/R/GitHub/LarvaeTransGen2018/data/JarCarbChemSummaryAllBots_6-1.csv")
```
...
eggdiam1<- lmer(EggDiamum~ParentTrt*AdultLength + (1|FemaleID)  + (1|AccTankID), data=CalEggs)
...
#Conclusion: Female matters but not treatment or adult length. Acclimation tank and treatment tank also don't matter.  
plot(EggDiamum~FemaleID, data=CalEggs) #we need to account for FemaleID, but not egg size in Larvae models. 
eggsizeSEs<- aggregate(EggDiamum~FemaleID, data=CalEggs, FUN=se)
max(eggsizeSEs$EggDiamum)
#test shape of eggs using eccentricity
...
#get the number of jars in each treatment
...
summary(larvlenfin)
r.squaredGLMM(larvlenfin)
```

# Look at larvae growth data
```{r LarvGrowthStat}
larvgrow<-lmer((LarvaeDiamum/3)~ParentTrt*JarSatArg_Total+(1|MaleID)+(1|CrossID)+(1|JarID) +(1|JarSeatable)+(1|TrtTankID)+(1|MaleTrtTankID) + (1|FemaleID), data=LarvaeDat)
steppedlarvgrow<- step(larvgrow)
print(steppedlarvgrow)
#final model chosen by the lme4 step function: y~SatCalcite_ATDIC*JarTrt+ (1|MaleID) + (1|FemaleID) + (1|JarID) + (1|JarSeatable)
larvgrowfin<- lmer((LarvaeDiamum/3)~ParentTrt*JarSatArg_Total+(1|MaleID) + (1|FemaleID)+ (1|JarID) + (1|JarSeatable), data=LarvaeDat)
#check the final model to see that it meets assumptions
...
summary(larvgrowfin)
r.squaredGLMM(larvgrowfin)

summary(larvgrowfin)
larvsize<-lmer(GrowthPerDay~ParentTrt*JarSatArg_Total+(1|MaleID)+(1|CrossID)+(1|JarID) +(1|JarSeatable)+(1|TrtTankID)+(1|MaleTrtTankID), data=LarvaeDat)
steppedlarvsize<- step(larvsize)
print(steppedlarvsize)
#final model chosen by the lme4 step function: y~ParentTrt*JarSatArg + (1|CrossID) + (1|JarID) + (1|JarSeatable)
larvsizefin<- lmer(GrowthPerDay~ParentTrt*JarSatArg_Total+(1|CrossID) + (1|JarID) + (1|JarSeatable), data=LarvaeDat)
#check the final model to see that it meets assumptions
plot(larvsizefin) #seems to meet assumption of linearity and homoscedasticity
qqnorm(resid(larvsizefin))
qqline(resid(larvsizefin)) #has relatively long tails, based on scale I think it reasonably meets assumption of normality. 
acf(resid(larvsizefin))
summary(larvsizefin)
r.squaredGLMM(larvsizefin) 
ggplot(LarvaeDat, aes(x=(LarvaeDiamum/3),color=JarTrt))+
  geom_histogram(alpha=0.5, position="identity")+
  facet_grid(ParentTrt~.)
...
## Look at larvae surface area
Parent Trt as fixed effect and Jar Treatment as covariate
```{r SurfaceAreaStat}
...
# Test larvae perimeter:length ratio
```{r LarvaeShapeStat}
...
summary(PerDiRatFin)
r.squaredGLMM(PerDiRatFin) #get R2; use R2c
```
...
#get final model R2 values
r.squaredGLMM(larvgrowfin) #GPD R2 use R2c
r.squaredGLMM(larvareafint) #area R2 use R2c
r.squaredGLMM(larvlenfin) #length R2 use R2c
r.squaredGLMM(larvperimfin) #perimeter R2 use R2c
...
```{r MultipanelFigure}
jpeg(paste0("figure3.jpeg"), height = 500, width = 500)
par(mfcol=c(2,2), mar=c(5.1, 4.1, 4.1, 2.1), xpd=F, pty="s")


p1<- visreg(larvsizefin, "JarSatArg_Total", by="ParentTrt", overlay=TRUE, ylab=expression(paste(Delta, " size (", mu, "m ", day^-1, ")")), xlab="", legend=FALSE, line=list(col=c( "#F7645A", "#004D4F"), lwd=c(2,2)),
              fill=list(col=c("#FCCECB", "#3BFAFF")),
        points=list(col=c("#F98880", "#007367"), pch=19, cex=0.4), cex.lab=1.5, cex.axis=1.25)
abline(a=0, b=0, col="black")
mtext("(a)", side=3, adj=0, padj=0, outer=F, at= -0.2, cex=1.5)
#legend.enhanced("topright",c("Control", "OA-exposed"),   pch=19,   col=c("#007367","#F98880"),   bty="n",   title="Parent Treatment",   horiz=TRUE,  y.intersp=.7,   x.intersp=0.5,   title.adj=c(.5,0),   title.col="black",   seg.len =1,   cex=.7,   yjust=0.5,   text.font = NULL,   inset = c(0,-.15)   )

p3<-visreg(larvareafint, "JarSatArg_Total", by="ParentTrt", overlay=TRUE, ylab=expression(sqrt(paste("Area (", mu, m^2,")"))), xlab=expression(paste("Larvae Treatment ", Omega[Aragonite])), legend=FALSE, cex=0.6, line=list(col=c( "#F7645A", "#004D4F"), lwd=c(2,2)),
              fill=list(col=c("#FCCECB", "#3BFAFF")),
        points=list(col=c("#F98880", "#007367"), pch=19, cex=0.4), cex.lab=1.5, cex.axis=1.25)
mtext("(c)", side=3, adj=0, padj=0, outer=F, at= -0.2, cex=1.5)

p2<-visreg(larvlenfin, "JarSatArg_Total", by="ParentTrt", overlay=TRUE, ylab=expression(paste("Length ", "(", mu, "m)")),  xlab="", legend=FALSE, line=list(col=c( "#F7645A", "#004D4F"), lwd=c(2,2)), fill=list(col=c("#FCCECB", "#3BFAFF")), points=list(col=c("#F98880", "#007367"), pch=19, cex=0.4), cex.lab=1.5, cex.axis=1.25)
mtext("(b)", side=3, adj=0, padj=0, outer=F, at= -0.2, cex=1.5)

p4<-visreg(larvperimfin, "JarSatArg_Total", by="ParentTrt", overlay=TRUE, ylab=expression(paste("Perimeter (", mu, "m)")),  xlab=expression(paste("Larvae Treatment ", Omega[Aragonite])), legend=FALSE, cex=0.6, line=list(col=c( "#F7645A", "#004D4F"), lwd=c(2,2)),
              fill=list(col=c("#FCCECB", "#3BFAFF")),
        points=list(col=c("#F98880", "#007367"), pch=19, cex=0.4), cex.lab=1.5, cex.axis=1.25)
mtext("(d)", side=3, adj=0, padj=0, outer=F, at= -0.2, cex=1.5)
dev.off()

#par(mfcol=c(1,1), xpd=F)
```

```{r get figure pdfs}

pdf(paste0("fig3shellgrowth.pdf"), height = 5, width = 5)
visreg(larvgrowfin, "JarSatArg_Total", by="ParentTrt", overlay=TRUE, ylab=expression(paste("Shell growth (", mu, "m/day)")), xlab="", legend=FALSE, line=list(col=c( "#F7645A", "#004D4F"), lwd=c(2,2)),
              fill=list(col=c("#FCCECB", "#3BFAFF")),xlab=expression(paste("Larvae Treatment ", Omega[Aragonite])),
        points=list(col=c("#F98880", "#007367"), pch=19, cex=0.4))
dev.off()



pdf(paste0("fig3size.pdf"), height = 5, width = 5)
visreg(larvsizefin, "JarSatArg_Total", by="ParentTrt", overlay=TRUE, ylab=expression(paste(Delta, "size (", mu, "m/day)")), xlab="", legend=FALSE, line=list(col=c( "#F7645A", "#004D4F"), lwd=c(2,2)),
              fill=list(col=c("#FCCECB", "#3BFAFF")),xlab=expression(paste("Larvae Treatment ", Omega[Aragonite])),
        points=list(col=c("#F98880", "#007367"), pch=19, cex=0.4))
abline(a=0, b=0, col="black")
dev.off()

pdf(paste0("fig3b.pdf"), height = 5, width = 5)
visreg(larvareafint, "JarSatArg_Total", by="ParentTrt", overlay=TRUE, ylab=expression(sqrt(paste("Area (", mu, m^2,")"))), xlab="", legend=FALSE, cex=0.6, line=list(col=c( "#F7645A", "#004D4F"), lwd=c(2,2)),
              fill=list(col=c("#FCCECB", "#3BFAFF")),
        points=list(col=c("#F98880", "#007367"), pch=19, cex=0.4))
dev.off()

pdf(paste0("fig3c.pdf"), height = 5, width = 5)
visreg(larvperimfin, "JarSatArg_Total", by="ParentTrt", overlay=TRUE, ylab=expression(paste("Perimeter (", mu, "m)")),  xlab=expression(paste("Larvae Treatment ", Omega[Aragonite])), legend=FALSE, cex=0.6, line=list(col=c( "#F7645A", "#004D4F"), lwd=c(2,2)),
              fill=list(col=c("#FCCECB", "#3BFAFF")),
        points=list(col=c("#F98880", "#007367"), pch=19, cex=0.4))
dev.off()
```

# Get ratio of OA to control to understand differences in calcification by family
...
segpd<- subset(seJarTrt, select= c(JarTrt, ParentTrt, MeanGPD, LarvaeDiamum, LarvaeAreaum2, LarvaePerimeterum))
mgpd$MeanGPD[1]/mgpd$MeanGPD[2]
mgpd$MeanGPD[3]/mgpd$MeanGPD[4]
...
mgpd$LarvaeDiamum[1]/mgpd$LarvaeDiamum[2]
mgpd$LarvaeDiamum[3]/mgpd$LarvaeDiamum[4]
...
#get difference in growth per day within parental treatments
...
# Get inter-family variability descriptive stats
```{r FamilyDescriptiveStats}
FamDat<- ddply(LarvByJarDat, .(CrossID, ParentTrt, JarTrt, MaleID, FemaleID), numcolwise(mean, na.rm=TRUE)) %>% 
  subset(select=c(CrossID, ParentTrt, JarTrt, MaleID, FemaleID, LarvaeSurvived, MeanGPD,LarvaeDiamum, LarvaePerimeterum, LarvaeAreaum2))  #calculate means for families using values from the three replicate jars in each treatment for each family; remove columns that are not of interest for calculating percent changes. 

FamDatToWide<- unite(FamDat, Value, c(LarvaeSurvived, MeanGPD,LarvaeDiamum, LarvaePerimeterum, LarvaeAreaum2), sep="_") #combine the growth and morphology columns to make them wide. 
FamWideDat<- spread(FamDatToWide, JarTrt, value= Value)

#now split the values for growth and morphology again. 
FamWideDat<- separate(FamWideDat, Control, into= c("MeanLarvSurvivedCon", "MeanGPDCon", "MeanLarvaeDiamCon", "MeanLarvaePerimCon", "MeanLarvaeAreaCon"), sep="_")
FamWideDat<- separate(FamWideDat, Exposed, into= c("MeanLarvSurvivedExp", "MeanGPDExp", "MeanLarvaeDiamExp", "MeanLarvaePerimExp", "MeanLarvaeAreaExp"), sep="_")

FamWideDat[,c(5:14)]<- sapply(FamWideDat[,c(5:14)], as.numeric) #save the values as numeric

#now calculate the percent change for each family
FamWideDat$GPDPercDiff<- with(FamWideDat, (MeanGPDExp-MeanGPDCon)/MeanGPDCon)
FamWideDat$LengthPercDiff<- with(FamWideDat, (MeanLarvaeDiamExp-MeanLarvaeDiamCon)/MeanLarvaeDiamCon)
FamWideDat$PerimPercDiff<- with(FamWideDat, (MeanLarvaePerimExp-MeanLarvaePerimCon)/MeanLarvaePerimCon)
FamWideDat$SAPercDiff<- with(FamWideDat, (MeanLarvaeAreaExp-MeanLarvaeAreaCon)/MeanLarvaeAreaCon)
FamWideDat$SurvPercDiff<- with(FamWideDat, (MeanLarvSurvivedExp-MeanLarvSurvivedCon)/MeanLarvSurvivedCon)

min(FamWideDat$GPDPercDiff)
max(FamWideDat$GPDPercDiff)
mean(FamWideDat$GPDPercDiff)
```


# Visualize variance among families: Calcification
Use the mean for each jar
...
#pull means for exposed larvae control parents
...
LarvByJarVarVizDat$ParentTrt<- factor(LarvByJarVarVizDat$ParentTrt, levels= c("400", "2600"), labels= c("", " "))

par(mar= c(1, 1, 0.5,0.5))
boxsize<-  ggplot(LarvByJarVarVizDat, aes(y=MeanGPD,x=as.factor(MaleIDNumber), color=JarTrt))+
  
  
  scale_fill_manual(values =c("#B1FDFF", "#FCCECB"))+
   geom_boxplot(alpha=0.5, position="identity") +
  labs(x = "", y= expression(paste(Delta, " size (", mu, "m ", day^-1, ")")), title = "Dam ID") +
  ...
  geom_hline(yintercept=ConExpMeans$MeanGPD, color="red", lty=2)+
  geom_boxplot(alpha=0.5, position="identity") +
  ...
  theme(axis.title.y.right = element_blank(),                # hide right axis title
        ...
        strip.background = element_rect(colour="white", fill="white"), axis.text.x=element_text(angle=90),plot.title = element_text(size=12, hjust = 0.5, vjust=-8), panel.border= element_blank(), legend.position="none", text= element_text(size=12)) 
  
boxlenday<- ggplot(LarvByJarVarVizDat, aes(y=(LarvaeDiamum),x=as.factor(MaleIDNumber), color=JarTrt))+
  geom_boxplot(alpha=0.5, position="identity") +
  facet_grid(~ParentTrt*FemaleIDNumber, scale="free_x")+ 
  geom_rect(aes(xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=Inf, fill = ParentTrt), colour=NA)+
  scale_fill_manual(values =c("#B1FDFF", "#FCCECB"))+
   geom_boxplot(alpha=0.5, position="identity") +
  labs(x = "", y= expression(paste("Length (", mu, "m)"))) +
  scale_color_manual(name = "Larval Treatment", labels = c("Control", "OA"), values = c("#004D4F", "#F7645A"))+
  geom_hline(yintercept=ConConMeans$LarvaeDiamum, color="navyblue", lty=2)+
...
boxarea<- ggplot(LarvByJarVarVizDat, aes(y=LarvaeAreaum2,x=as.factor(MaleIDNumber), color=JarTrt))+
  geom_boxplot(alpha=0.5, position="identity") +
  facet_grid(~ParentTrt*FemaleIDNumber, scale="free_x")+ 
  geom_rect(aes(xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=Inf, fill = ParentTrt), colour=NA)+
  scale_fill_manual(values =c("#B1FDFF", "#FCCECB"))+
   geom_boxplot(alpha=0.5, position="identity") +
  labs(x = "", y= expression(paste("Surface Area (", mu, m^2,")"))) +
  scale_color_manual(name = "Larval Treatment", labels = c("Control", "OA"), values = c("#004D4F", "#F7645A"))+
  ...
  theme(axis.title.y.right = element_blank(),                # hide right axis title
        ...
        axis.text.y = element_text(margin = margin(r = 0)),  # move left axis labels closer to axis
        panel.spacing = unit(0, "mm"),                    # remove spacing between facets
        strip.background = element_rect(colour="white", fill="white"), axis.text.x=element_text(angle=90),plot.title = element_text(size=12, hjust = 0.5, vjust=-8), panel.border= element_blank(), text= element_text(size=12), legend.position = "none")

boxdiam<- ggplot(LarvByJarVarVizDat, aes(y=LarvaeDiamum ,x=MaleIDNumber, color=JarTrt))+
   geom_boxplot(alpha=0.5, position="identity") +
  facet_grid(~ParentTrt*FemaleIDNumber, scale="free_x")+ 
  geom_rect(aes(xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=Inf, fill = ParentTrt), colour=NA)+
  scale_fill_manual(values =c("#B1FDFF", "#FCCECB"))+
   geom_boxplot(alpha=0.5, position="identity") +
  labs(x = "Sire ID", y= expression(paste("Length (", mu, "m)")), title = "Dam ID") + 
    scale_color_manual(name = "Larval Treatment", labels = c("Control", "OA"), values = c("#004D4F", "#F7645A"))+
  ...
  theme(axis.title.y.right = element_blank(),                # hide right axis title
        ...
        axis.text.y = element_text(margin = margin(r = 0)),  # move left axis labels closer to axis
        panel.spacing = unit(0, "mm"),                    # remove spacing between facets
        strip.background = element_rect(colour="white", fill="white"), axis.text.x=element_text(angle=90),plot.title = element_text(size=14, hjust = 0.5, vjust=-8), panel.border= element_blank(), text= element_text(size=14), legend.position = "none")

boxperim<- ggplot(LarvByJarVarVizDat, aes(y=LarvaePerimeterum ,x=MaleIDNumber, color=JarTrt))+
    geom_boxplot(alpha=0.5, position="identity") +
  facet_grid(~ParentTrt*FemaleIDNumber, scale="free_x")+ 
  geom_rect(aes(xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=Inf, fill = ParentTrt), colour=NA)+
  scale_fill_manual(values =c("#B1FDFF", "#FCCECB"))+
   geom_boxplot(alpha=0.5, position="identity") + 
  labs(x = "Sire ID", y= expression(paste("Perimeter (", mu, "m)"))) + 
      scale_color_manual(name = "Larval Treatment", labels = c("Control", "OA"), values = c("#004D4F", "#F7645A"))+
...
boxshape<- ggplot(LarvByJarVarVizDat, aes(y=(LarvaePerimeterum/LarvaeDiamum),x=MaleIDNumber, color=JarTrt))+
  geom_boxplot(alpha=0.5, position="identity") +
  facet_grid(~ParentTrt*FemaleIDNumber, scale="free_x")+ 
  geom_rect(aes(xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=Inf, fill = ParentTrt), colour=NA)+
  scale_fill_manual(values =c("#B1FDFF", "#FCCECB"))+
   geom_boxplot(alpha=0.5, position="identity") +
  labs(x = "Sire ID", y= paste("Perimeter:Length"), title = "Dam ID") + 
    scale_color_manual(name = "Larval Treatment", labels = c("Control", "OA"), values = c("#004D4F", "#F7645A"))+
    theme_classic() +
  theme(axis.title.y.right = element_blank(),                # hide right axis title
        axis.text.y.right = element_blank(),                 # hide right axis labels
...
BoxPlotFig <- ggarrange(boxsize, boxlenday  , boxarea, boxperim,
                    labels = c("(a)", "(b)", "(c)", "(d)"),
                    ncol = 1, nrow = 4)

annotate_figure(BoxPlotFig,
               top = text_grob("Control parents                                      Exposed parents", size = 14, hjust=0, x=0.25)
               
              )
```
```{r}
jpeg("Boxplotfigure.jpeg", height= 1000, width= 700)
BoxPlotFig
annotate_figure(BoxPlotFig,
               top = text_grob("Control parents                                                Exposed parents", size = 14, hjust=0, x=0.25, vjust=2.5))
...
```{r}
pdf(paste0("fig4a.pdf"), height = 5, width = 10)
...
pdf(paste0("fig4b.pdf"), height = 5, width = 10)
...
pdf(paste0("fig4c.pdf"), height = 5, width = 10)
...
ParTreat<- c("400"="Control Parents", "2600"="Exposed Parents")
LarvTreat<- c("Control"= "Control", "Exposed" = "Exposed")
jpeg("Fig5.jpeg", height = 600, width= 700)
ggplot(SurvDat,aes(x=JarTrt, y=LarvaeSurvived, color = "black45", group= CrossID)) +
  ...
  geom_errorbar(aes(ymin= LarvaeSurvived-SELarvaeSurvived, ymax=LarvaeSurvived+SELarvaeSurvived),width=0.05, position=position_dodge(0))+
    labs(x = "Larvae Treatment", y = "Number of larvae survived", title= "Parent Treatment") +
  ...
  theme(panel.spacing = unit(0.2, "mm"),                    # remove spacing between facets
        strip.background = element_rect(colour="white", fill="white"), panel.border= element_blank(), legend.position="none", text= element_text(size=14), plot.title = element_text(size=14, hjust = 0.5), strip.text.x = element_text(size = 14), axis.text=element_text(size=14))
dev.off()
#make bar graph of survival
ggplot(data = MeanSurvDat, aes(x = ParentTrt, y = RatSurv, fill=ParentTrt)) +
  ...
  scale_fill_manual(values =c("#004D4F", "#F7645A"))+
...
# Look at trade offs between growth and survival

## Survival ratio and GPD in exposed conditions
One point for each family with SE for x and y axes
...
ExpFams$MeanGPDExp2<- ExpFams$MeanGPDExp^2 #make a squared variable
ExpFams$LogMeanGPDExp<- log(ExpFams$MeanGPDExp)
EPmodlm<- lmodel2(RatSurv~MeanGPDExp, data=ExpFams, nperm= 1000)
EPmodlm2<- lmodel2(RatSurv~MeanGPDExp+ MeanGPDExp2, data=ExpFams, nperm= 1000)
EPmodlm3<- lmodel2(RatSurv~MeanGPDExp2, data= ExpFams, nperm=1000)
EPmodlm4<- lmodel2(RatSurv~MeanGPDExp + LogMeanGPDExp, data=ExpFams, nperm=1000)
EPmodlm5<- lmodel2(RatSurv~ LogMeanGPDExp, data=ExpFams, nperm=1000)
...
par(mfcol=c(2,2))
plot(EPmodlm)
ConFams$MeanGPDExp2<- ConFams$MeanGPDExp^2
EPmodlmcon<- lmodel2(RatSurv~MeanGPDExp, data=ConFams, nperm=1000)

EPmodlmcon2<- lmodel2(RatSurv~MeanGPDExp + MeanGPDExp2, data=ConFams, nperm=1000)
...
EPmodlm
EPmodlmcon
EPmodlmconTidy<- tidy(EPmodlmcon)
...
plot(RatSurv~MeanGPDCon, data=ConFams, pch=19, ylim=c(.5,1.3), xlim=c(19,27))
points(RatSurv~MeanGPDCon, data=ExpFams, pch=19, col="pink")

plot(EPmodlmcon, "OLS")

par(mar=c(5,6,1,1), mfrow=c(1,1), xpd=F)
plot(RatSurv~MeanGPDExp, data=ConFams, pch=19, col="#007367",ylim=c(.5,1.3), xlim=c(-1.9, 2.4), xlab= expression(paste("Mean Family Growth (", mu, "m", day^-1, " )")), ylab="Proportion of Surviving Larvae \n (OA:Control)")
...
points(RatSurv~MeanGPDExp, data=ConFams, pch=19, col="#004D4F")
points(RatSurv~MeanGPDExp, data=ExpFams, pch=19, col="#F7645A")
ablineclip(a=-0.1018317, b=0.04652294, x1=min(ConFams$MeanGPDExp), x2=max(ConFams$MeanGPDExp), col="#007367", lwd=2.5)
par(xpd=TRUE)
...
pdf("fig6.pdf", height = 5, width = 8)
par(mar=c(5,6,1,1), mfrow=c(1,1), xpd=F)
plot(RatSurv~MeanGPDExp, data=ConFams, pch=19, col="#007367",ylim=c(.5,1.3), xlim=c(19,24), xlab= expression(paste("Mean Family Growth (", mu, "m/day)")), ylab="Proportion of Surviving Larvae \n (OA:Control)")
arrows(x0=(SurvWideDat$MeanGPDExp-SurvWideDat$SEMeanGPDCExp), x1= (SurvWideDat$MeanGPDExp+SurvWideDat$SEMeanGPDCExp), y0 = SurvWideDat$RatSurv, y1= SurvWideDat$RatSurv, code=3, angle=90, len=.1)
points(RatSurv~MeanGPDExp, data=ConFams, pch=19, col="#004D4F")
points(RatSurv~MeanGPDExp, data=ExpFams, pch=19, col="#F7645A")
ablineclip(a=-0.1018317, b=0.04652294, x1=min(ConFams$MeanGPDExp), x2=max(ConFams$MeanGPDExp), col="#007367", lwd=2.5)
...
```{r t-tests}
#get Bonferoni adjusted p-critical value: 
0.05/10
...
t.test((LarvaeDiamum/3)~ParentTrt, data=Lexp, var.equal=TRUE) 
t.test((LarvaeDiamum/3)~ParentTrt, data=Lcon, var.equal=TRUE)
...
# Survival protocol versions check
...
```{r Sire and dam effects}
par(mfcol=c(1,1), xpd=F)
#get mean for each dam and each sire growth per day
SireMeans<- ddply(LarvaeDat, .(MaleID, JarTrt), numcolwise(mean, na.rm=T))
DamMeans<- ddply(LarvaeDat, .(FemaleID, JarTrt), numcolwise(mean, na.rm=T))

SireMeansCont<- filter(SireMeans, JarTrt=="Control")
SireMeansExp<- filter(SireMeans, JarTrt=="Exposed")
DamMeansCont<- filter(DamMeans, JarTrt=="Control")
DamMeansExp<- filter(DamMeans, JarTrt=="Exposed")

#add these to the larvaedat data frame
LarvByJarVarVizDat$SireGPDCont<- SireMeansCont$GrowthPerDay[match(LarvByJarVarVizDat$MaleID, SireMeansCont$MaleID)]
LarvByJarVarVizDat$SireGPDExp<- SireMeansExp$GrowthPerDay[match(LarvByJarVarVizDat$MaleID, SireMeansExp$MaleID)]
LarvByJarVarVizDat$DamGPDCont<- DamMeansCont$GrowthPerDay[match(LarvByJarVarVizDat$FemaleID, DamMeansCont$FemaleID)]
LarvByJarVarVizDat$DamGPDExp<- DamMeansExp$GrowthPerDay[match(LarvByJarVarVizDat$FemaleID, DamMeansExp$FemaleID)]

FamMeansWide$MaleID<- LarvByJarDat$MaleID[match(FamMeansWide$CrossID, LarvByJarDat$CrossID)]
FamMeansWide$FemaleID<- LarvByJarDat$FemaleID[match(FamMeansWide$CrossID, LarvByJarDat$CrossID)]
FamMeansWide$SireGPDCont<- SireMeansCont$GrowthPerDay[match(FamMeansWide$MaleID, SireMeansCont$MaleID)]
FamMeansWide$SireGPDExp<- SireMeansExp$GrowthPerDay[match(FamMeansWide$MaleID, SireMeansExp$MaleID)]
FamMeansWide$DamGPDCont<- DamMeansCont$GrowthPerDay[match(FamMeansWide$FemaleID, DamMeansCont$FemaleID)]
FamMeansWide$DamGPDExp<- DamMeansExp$GrowthPerDay[match(FamMeansWide$FemaleID, DamMeansExp$FemaleID)]

ggplot(FamMeansWide, aes(y=MeanGPD_Control, x=SireGPDCont, color= FemaleID))+
  geom_point()+
  geom_abline(slope=1, intercept=0)+ 
  geom_smooth(method=lm, se=FALSE, fullrange=F)+
   labs(x = "Sire Mean GPD Control", y= "Cross GPD Control") + 
  theme_classic()
ggplot(FamMeansWide, aes(y=MeanGPD_Exposed, x=SireGPDExp, color= FemaleID))+
  geom_point()+
  geom_abline(slope=1, intercept=0)+ 
  geom_smooth(method=lm, se=FALSE, fullrange=F)+
   labs(x = "Sire Mean GPD Exposed", y= "Cross GPD Exposed") + 
  theme_classic()
ggplot(FamMeansWide, aes(y=MeanGPD_Control, x=DamGPDCont, color= MaleID))+
  geom_point()+
  geom_abline(slope=1, intercept=0)+ 
  geom_smooth(method=lm, se=FALSE, fullrange=F)+
   labs(x = "Dam Mean GPD Control", y= "Cross GPD Control") + 
  theme_classic()
ggplot(FamMeansWide, aes(y=MeanGPD_Exposed, x=DamGPDExp, color= MaleID))+
  geom_point()+
  geom_abline(slope=1, intercept=0)+ 
  geom_smooth(method=lm, se=FALSE, fullrange=F)+
   labs(x = "Dam Mean GPD Exposed", y= "Cross GPD Exposed") + 
  theme_classic()
ggplot(FamMeansWide, aes(y=SireGPDExp, x=DamGPDExp))+
  geom_point()+ 
   labs(x = "Dam Mean GPD Exposed", y= "Sire Mean GPD Exposed") + 
  theme_classic()

```

```{r Dam and sire heat map}
#want to create a heatmap of difference in larval growth per day for each family. #first get just control families

FamMeansWide$FemIDPlain<- vapply(strsplit(as.character(FamMeansWide$FemaleID),"_"), `[`, 1, FUN.VALUE=character(1))
FamMeansWide$MaleIDPlain<- vapply(strsplit(as.character(FamMeansWide$MaleID),"_"), `[`, 1, FUN.VALUE=character(1))
FamMeansWide$MaleIDNumber<- substr(FamMeansWide$MaleIDPlain, 3, 4 )
FamMeansWide$FemaleIDNumber<- substr(FamMeansWide$FemIDPlain, 3, 4 )
FamMeansWide$ParTrt<- substr(FamMeansWide$MaleIDPlain, 1,1)
ConFamMeansWide<- filter(FamMeansWide, startsWith(as.character(FamMeansWide$CrossID), "C")== T)
ggplot(data = FamMeansWide) +                        # Set data
  geom_tile(aes(x = FemaleIDNumber, y = MaleIDNumber, fill = OACont_GPD)) +        # Define variables to plot
  scale_fill_gradient2(low = "darkred", mid="violet", high = "navyblue", midpoint=mean(ConFamMeansWide$OACont_GPD))+  # Set colour scheme
  facet_grid(~ParTrt, scale="free")+
  labs(x="Female ID", y= "Male ID", fill= "GPD Difference \n (OA-Control)")+
  theme_classic()
  
  ggplot(data = FamMeansWide) +                        # Set data
  geom_tile(aes(x = MaleIDNumber, y = FemaleIDNumber, fill = OACont_Len)) +        # Define variables to plot
  scale_fill_gradient2(low = "darkred", mid="violet", high = "navyblue", midpoint=mean(ConFamMeansWide$OACont_Len))+  # Set colour scheme
  facet_grid(~ParTrt, scale="free")+
  labs(x="Male ID", y= "Female ID", fill= "Length Difference \n (OA-Control)")+
  theme_classic()
  
ggplot(data = FamMeansWide) +                        # Set data
  geom_tile(aes(x = FemaleIDNumber, y = MaleIDNumber, fill = OACont_SA)) +        # Define variables to plot
  scale_fill_gradient2(low = "darkred", mid="violet", high = "navyblue", midpoint=mean(ConFamMeansWide$OACont_SA))+  # Set colour scheme
  facet_grid(~ParTrt, scale="free")+
  labs(x="Female ID", y= "Male ID", fill= "Surface Area Difference \n (OA-Control)")+
  theme_classic()

  ggplot(data = FamMeansWide) +                        # Set data
  geom_tile(aes(x = FemaleIDNumber, y = MaleIDNumber, fill = OACont_P)) +        # Define variables to plot
  scale_fill_gradient2(low = "darkred", mid="violet", high = "navyblue", midpoint=mean(ConFamMeansWide$OACont_P))+  # Set colour scheme
  facet_grid(~ParTrt, scale="free")+
  labs(x="Female ID", y= "Male ID", fill= "Perimeter Difference \n (OA-Control)")+
  theme_classic()

```

# Effect size/importance of faster growth rate 

```{r Extrapolate growth per day}
#larvae that are >202 um are closer to the bottom, so let's use that size and see how long larvae from each treatment take to get there

#make a new column that is days to 202 for each larvae jar
LarvByJarDat$Daysto202 <- (202-LarvByJarDat$LarvaeDiamum)/LarvByJarDat$MeanGPD
#get the mean number of days for each treatment
aggregate(MeanGPD~JarTrt*ParentTrt, data= LarvByJarDat, FUN=mean)
aggregate(LarvaeDiamum~JarTrt*ParentTrt, data= LarvByJarDat, FUN=mean)
```
# get number of eggs per female
```{r}
table(Egg_Morphology$FemaleID)
```


```{r get standard error of larvae measurements}
larvaemeasSEs<- aggregate(LarvaeDiamum~JarID, data=LarvaeDat, FUN= se)

```

# Get salinity to alkalinity relationship
```{r Sal to At relationship}
SalAT<- lm(Alk_corr~salinity, data=Tank_WC)
par(mfcol=c(2,2))
plot(SalAT)
par(mfcol=c(1,1))
summary(SalAT)
plot(Alk_corr~salinity, data=Tank_WC)
```
